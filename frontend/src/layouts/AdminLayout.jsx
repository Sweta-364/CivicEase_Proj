...
import { Building2, Layers3, LibraryBig, LogOut, HelpCircle, ArrowLeft } from 'lucide-react';
...
const adminNavItems = [
  { to: '/admin/issues', label: 'Admin Issues', icon: Layers3, check: 'adminIssues' },
  { to: '/admin/departments', label: 'Departments', icon: Layers3, check: 'mainAdmin' },
  { to: '/admin/clusters', label: 'Clusters', icon: Layers3, check: 'mainAdmin' },
  { to: '/admin/resources/new', label: 'Publish Resource', icon: LibraryBig, check: 'adminResource' },
];
        ...
        <div className="h-full p-5 shrink-0">
          ...
          >
            ...
            <div className="p-8 flex items-center gap-3">
              ...
              {!sidebarCollapsed && <span className="text-lg font-bold text-slate-900 tracking-tight">CivicEase <span className="text-sky-600">Admin</span></span>}
            </div>
            ...
            <div className="px-3 py-3 space-y-1">
              <button
                className={`group w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sky-700 hover:text-sky-900 hover:bg-sky-50/50 transition-all duration-200 ease-in-out ${sidebarCollapsed ? 'justify-center' : ''}`}
                title={sidebarCollapsed ? 'Back to User Panel' : ''}
                onClick={() => navigate('/home')}
              >
                <div className="group-hover:scale-105 group-hover:text-sky-700 transition-all duration-200">
